...
public class StartTrotty {
	...
	public static void main(String[] args) {
		...
		HashMap<String, Integer> c = wifi.run();
		int role = 1;
		if((int)((c.get("BTN"))) == 2)
			role = 1; //builder
		...
		do {
			...
			t.drawString("< Left : LocalUS >", 0, 0);
			t.drawString(" ENTER: Find Blocks ", 0, 1);
			t.drawString("       |        ", 0, 2);
			t.drawString("       |        ", 0, 4);
			t.drawString("       |        ", 0, 5);
			t.drawString("       |        ", 0, 6);
		...
		if (buttonChoice == Button.ID_LEFT) {
			...
			odo.start();
			Capture capture = new Capture(clawMotor, leftMotor, rightMotor);
			usPollerF.start();
			USLocalizer usl = new USLocalizer(odo, usPollerF, USLocalizer.LocalizationType.FALLING_EDGE, navigator, wifi.getCorner());
			...
			usPollerS.start();
			ObjectFinder of = new ObjectFinder(leftMotor, rightMotor, navigator, odo,
					usPollerF, usPollerS, usl, lsPoller, resolution, wifi.getXHalf(), wifi.getYHalf(), capture);
			of.setEndCoords();
			of.setDepositCoords(wifi.getDepositZoneX(), wifi.getDepositZoneY());
			of.pointDriver();
		} 
		System.exit(0);	
}
